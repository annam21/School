  # Anna Moeller
  # Lab day 3 - RMark with dipper data for CJS
  # 9/15/2016

  data(dipper)
  # This is a demonstration that you can run default models without knowing what you're doing
  dipper.model <- mark(dipper) # Use defaults: CJS model, phi(.).p(.)
  
  # Make a function to run all of our models
  run.dipper <- function(){
  ...
  # To look at a single model
  dipper.results$dipper.phidot.pdot
  
  # Some prettier results
  get.real(dipper.results$dipper.phidot.pdot, se = T, parameter = "Phi")
  
  # Run a goodness of fit test (on processed data)
  release.gof(dipper.processed)
  # No evidence of lack of fit (p > 0.05)
  
# Part 2 of lab: run a different set of models
  ...
  # Call second round of models
  dipper.results.alternate <- run.dipper.alternate()
  #
  # Merge all of our models so far into a single output list 
  #   (first round, second round, default model)
